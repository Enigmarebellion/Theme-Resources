...
declare -a res=('Android.mk' 'AndroidManifest.xml' 'MODULE_LICENSE_APACHE2' 'NOTICE' 'lint.xml' 'assets' 'menu*' 'values-*' 'xml*' 'interpolator' 'raw*' 'anim*' 'animator' 'transition' 
'MakeJavaSymbols.sed' 'tests' 'src' 'build.gradle' 'proguard.flags' 'setting' 'gradlew' 'gradlew.bat')
...
declare -a values=('all_search_engines.xml' 'appmsg_colors.xml' 'arrays.xml' 'attrs_manifest.xml' 'bookmarks_icons.xml' 'bools.xml' 'config.xml' 'defaults.xml' 'donottranslate.xml' 'donottranslate_config.xml'
                   ...
                   'donottranslate-search_engines.xml' 'rr_arrays.xml' 'rr_strings.xml' 'rr_symbols.xml' 'fractions.xml' 'ids.xml' 'integers.xml' 'internal.xml' 'keys.xml' 'lland_config.xml' 'lland_strings.xml' 'plurals.xml'
...
declare -a theme_packages=('Calculator' 'Calendar' 'CalendarWidget' 'Camera2' 'Contacts' 'ContactsCommon' 'DUI' 'DeskClock' 'Dialer' 'Gallery2'
                           'Launcher3' 'Messaging' 'MusicFX' 'OmniSwitch' 'PhoneCommon' 'Settings' 'Stk' 'ResurrectionOTA' 'ResurrectionStats' )
...
echo "Copying all files to $WORKING_DIR/vendor/scripts/themes-resources"
...
echo "ResurrectionRemix Theme Resources Update on $(date)" >> /tmp/thememsg
